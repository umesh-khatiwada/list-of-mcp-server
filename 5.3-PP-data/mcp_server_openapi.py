    ...
    try:
        ...
        if not redis_url:
            ...
            error_result = json.dumps(
                {"error": "REDIS_URL environment variable not set", "key": key}
            )
            return error_result
        ...
        try:
            ...
            if token:
                result = json.dumps(
                    {"success": True, "token": token, "key": key, "source": "redis"}
                )
                logger.debug(
                    f"[REDIS] Successfully retrieved token from Redis for key '{key}': {result}"
                )
                return result
            else:
                logger.debug(
                    f"[REDIS] Token not found in Redis for key '{key}', using mock data"
                )
            ...
            logger.debug(
                f"[REDIS] Redis connection error: {redis_error}, falling back to mock data"
            )
            mock_result = json.dumps(
                {
                    "success": True,
...
def with_token_middleware(tool_func):
    ...
    async def wrapper(*args, **kwargs) -> str:
        logger.debug(
            f"[MIDDLEWARE] Tool '{tool_func.__name__}' called with args: {args}, kwargs: {kwargs}"
        )
        ...
        # Get token from Redis
        logger.debug(
            f"[MIDDLEWARE] Fetching token from Redis for sessionId: '{sessionId}'"
        )
        token_result = await redis_get_token(sessionId)
        try:
            if token_result.startswith('{"error"'):
                logger.error(
                    f"[MIDDLEWARE] Redis error for sessionId '{sessionId}': {token_result}"
                )
            ...
            if token_data.get("success") and token_data.get("token"):
                ...
                if sessionId not in with_token_middleware._session_contexts:
                    with_token_middleware._session_contexts[
                        sessionId
                    ] = contextvars.ContextVar(f"auth_headers_{sessionId}")
                ...
                if not hasattr(with_token_middleware, "_current_session_context"):
                    with_token_middleware._current_session_context = (
                        contextvars.ContextVar("current_session")
                    )

                with_token_middleware._current_session_context.set(sessionId)
                result = tool_func(*args, **kwargs)
                logger.debug(
                    f"[MIDDLEWARE] Tool '{tool_func.__name__}' completed for sessionId: '{sessionId}'"
                )
                return result
            else:
...
def make_authenticated_request(
    ...
    return make_api_request(
        ...
        params=params,
        json_data=json_data,
        headers_override=dynamic_headers,
    )


def make_authenticated_api_request(
    method: str,
    endpoint: str,
    params: Optional[Dict] = None,
    json_data: Optional[Dict] = None,
    ...
    try:
        if hasattr(with_token_middleware, "_current_session_context"):
            ...
            if current_session and hasattr(with_token_middleware, "_session_contexts"):
                session_context = with_token_middleware._session_contexts.get(
                    current_session
                )
                if session_context:
...
def make_api_request(
    ...
    try:
        ...
        with httpx.Client(
            base_url=base_url, headers=request_headers, timeout=30.0
        ) as client:
            ...
            if "application/json" in content_type:
                ...
                logger.debug(
                    f"Text Response (first 200 chars): {response.text[:200]}..."
                )
...
def update_project(
    ...
    return make_authenticated_api_request(
        "PUT", f"/projects/{project_id}", json_data=data
    )
...
def create_environment(
    name: str, project_id: str, sessionId: str = None, region: Optional[str] = None
) -> str:
...
def restart_deployment(deployment_id: str, sessionId: str = None) -> str:
    ...
    return make_authenticated_api_request(
        "PUT", f"/deployments/{deployment_id}/restart"
    )
...
def get_deployment_logs(deployment_id: str, sessionId: str = None) -> str:
    ...
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/deploylogs"
    )
...
def get_deployment_runtime_logs(
    deployment_id: str, sessionId: str = None, lines: Optional[str] = None
) -> str:
    ...
    if lines:
        params["lines"] = lines
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/runtimelogs", params=params
    )
...
def get_deployment_metrics(
    ...
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/metrics", params=params
    )
...
def get_spherestor(
    ...
    return make_authenticated_api_request(
        "GET", f"/spherestor/{environment_id}/env", params=params
    )


@mcp.tool(name="create_spherestor")
@with_token_middleware
def create_spherestor(
    environment_id: str, name: str, storage_size: int, sessionId: str = None
) -> str:
    """Create new spherestor (storage) in an environment."""
    data = {"name": name, "storage_size": storage_size}
    return make_authenticated_api_request(
        "POST", f"/spherestor/{environment_id}", json_data=data
    )
...
@with_token_middleware
def create_team(
    name: str, sessionId: str = None, description: Optional[str] = None
) -> str:
    """Create a new team."""
    data = {"name": name}
    if description:
...
def create_api_token(
    name: str, scope: str, sessionId: str = None, expiry: Optional[str] = None
) -> str:
...
def list_payment_history(
    ...
    return make_authenticated_api_request(
        "GET", "/accounts/payments/history", params=params
    )
...
def get_payment_history(payment_id: str, sessionId: str = None) -> str:
    ...
    return make_authenticated_api_request(
        "GET", f"/accounts/payments/history/{payment_id}"
    )
...
def get_sphereops_spherelets_usage(
    ...
    return make_authenticated_api_request(
        "GET", "/sphereops/spherelets-usage", params=params
    )
...
def get_sphereops_storage_usage(
    ...
    return make_authenticated_api_request(
        "GET", "/sphereops/storage-usage", params=params
    )
...
def get_deployment_pod_count(
    ...
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/podCount", params=params
    )
...
def redeploy_deployment(deployment_id: str, sessionId: str = None) -> str:
    ...
    return make_authenticated_api_request(
        "PUT", f"/deployments/{deployment_id}/redeploy"
    )
...
def get_deployment_current_version(deployment_id: str, sessionId: str = None) -> str:
    ...
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/currentVersion"
    )
...
def list_deployment_histories(
    ...
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/histories", params=params
    )
...
def get_deployment_history(
    deployment_id: str, history_id: str, sessionId: str = None
) -> str:
    """Get specific deployment history."""
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/histories/{history_id}"
    )
...
def get_deployment_ci_build_logs(deployment_id: str, sessionId: str = None) -> str:
    ...
    return make_authenticated_api_request(
        "GET", f"/deployments/{deployment_id}/ciBuildlogs"
    )
