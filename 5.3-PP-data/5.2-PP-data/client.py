...
class MCPOpenAIClient:
    ...
    def __init__(self, model: str = "gemini-2.5-flash", memory_windows: int = 5):
        ...
        system_prompt = (
            "You are a helpful AI assistant that can access the Computesphere API.\n"
            "\n"
    ...
    def _needs_authentication(self, tool_name: str) -> bool:
        ...
        return tool_name in self.authenticated_tools
    ...
    async def process_query(self, query: str) -> str:
        ...
        if is_formatting_request and cached_response:
            ...
            format_messages = [
                ...
                {
                    "role": "user",
                    "content": f"Format this data according to the request '{query}':\n\n{cached_response}",
                },
            ]
        ...
        if tool_calls_made == 1:
            if assistant_message.tool_calls:
                ...
                if server:
                    print(
                        f"🔧 [1] Calling tool: {tool_call.function.name} with args: {tool_call.function.arguments}"
                    )

                    # Parse arguments and inject sessionId if needed
                    arguments = json.loads(tool_call.function.arguments)
                    arguments = self._inject_session_id_if_needed(
            ...
            if second_assistant_message.tool_calls:
                for tool_call in second_assistant_message.tool_calls:
                    ...
                    if server:
                        print(
                            f"🔧 [2] Calling tool: {tool_call.function.name} with args: {tool_call.function.arguments}"
                        )
            ...
            for i, tool_call in enumerate(assistant_message.tool_calls):
                ...
                if not server:
                    error_msg = (
                        f" Tool '{tool_call.function.name}' not found on any server."
                    )
                    self.memory.add_assistant_message(error_msg)
                    return error_msg

                print(
                    f"🔧 [{i+1}] Calling tool: {tool_call.function.name} with args: {tool_call.function.arguments}"
                )
        ...
        if api_response_data and api_call_info:
            ...
            print(
                f"💾 Cached API response for future formatting requests (Call ID: {self.memory.last_api_call['call_id'][:8]}...)"
            )
