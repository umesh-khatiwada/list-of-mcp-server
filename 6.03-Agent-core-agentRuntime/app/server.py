...
from pydantic import (  # RootModel is no longer strictly needed but keeping for now
    BaseModel,
    RootModel,
)
    ...
    try:
        ...
        if instruction == "k8s_resource_status":
            ...
            if resource == "pods":
                ...
                logger.warning(
                    f"Unsupported resource type for k8s_resource_status: {resource}"
                )
                return JSONResponse(
                    status_code=400,
                    content={"error": f"Unsupported resource: {resource}"},
                )
            ...
            if not pod_name:
                ...
                return JSONResponse(
                    status_code=400, content={"error": "Missing 'pod_name' parameter."}
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"error": f"Unknown instruction: {instruction}"},
            )

    except (
        json.JSONDecodeError
    ) as e:  # This block might not be hit now, but good to keep for robustness
        logger.error(f"JSON decoding error: {e}")
        return JSONResponse(
            status_code=400,
            content={"error": f"Invalid JSON format in request body: {e}"},
        )
    except client.ApiException as e:
        logger.error(
            f"Kubernetes API error (status {e.status}): {e.reason} - {e.body.decode('utf-8') if e.body else 'No response body'}"
        )
        return JSONResponse(
            status_code=e.status,
            content={
                "error": f"Kubernetes API error: {e.reason} - {e.body.decode('utf-8') if e.body else 'No response body'}"
            },
        )
    except Exception:
