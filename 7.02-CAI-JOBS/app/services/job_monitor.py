...
from ..services.session_store import save_sessions, sessions_store
                ...
                if "job_names" in session:
                    ...
                    if all_completed:
                        ...
                        for job_name in job_names:
                            (
                                ...
                                log_path,
                                log_content,
                            ) = k8s_service.get_manifestwork_logs(job_name)
                            if logs:
                        ...
                        combined_logs = (
                            "\n\n".join(all_logs)
                            if all_logs
                            else f"Multi-job session completed with statuses: {all_statuses}"
                        )
                    ...
                    if status in ["Completed", "Failed"]:
                        ...
                        (
                            ...
                            log_path,
                            log_content,
                        ) = k8s_service.get_pod_logs_with_path(session_id)
                        await send_webhook(
                            session_id,
                            session["jobName"],
                            status,
