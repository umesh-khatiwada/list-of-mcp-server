...
RSS_FEED_URL = os.getenv(
    "RSS_FEED_URL", "https://umeshkhatiwada.com.np/assets/file/cm.rss"
)
    ...
    async def load_rss_endpoints(self) -> List[APIEndpoint]:
        ...
        try:
            ...
            for i, entry in enumerate(feed.entries):
                ...
                print(
                    f"Entry {i+1}: Title='{title}', Description='{description[:100]}...'"
                )
                ...
                if any(
                    ...
                    for word in ["get", "fetch", "retrieve", "list", "show"]
                ):
                    method = "GET"
                elif any(
                    word in text_to_check for word in ["post", "create", "add", "send"]
                ):
                    method = "POST"
                elif any(
                    word in text_to_check
                    for word in ["put", "update", "modify", "edit"]
                ):
                    method = "PUT"
                ...
                print(
                    f"  -> Method: {method}, Path: '{path}', Keywords: {keywords[:3]}"
                )
                if not path:
                    path = "/api/v1/default"
        ...
        return json.dumps(
            {
                ...
                "endpoints": [
                    {"method": ep.method, "path": ep.path, "keywords": ep.keywords[:5]}
                    for ep in endpoints
                ],
            }
        )
...
@with_auth_middleware
async def perizer_data_curl_get(
    url: str, parameters2_Value: str, Authorization: str
) -> str:
...
async def perizer_data_curl_post(
    ...
    try:
        headers = session_manager.build_headers(
            parameters4_Value, content_type="application/json"
        )
        # Build request body
        body = {}
...
@with_auth_middleware
async def perizer_data_curl_put(
    url: str, parameters4_Value: str, JSON: dict, Authorization: str
) -> str:
...
@mcp.tool(
    name="ai_agent_process", description="Process user messages and execute API calls"
)
@with_auth_middleware
async def ai_agent_process(
    user_message: str, session_id: str, Authorization: str
) -> str:
    ...
    try:
        ...
        try:
            if best_match.method == "GET":
                ...
                return json.dumps(
                    {"error": f"Unsupported HTTP method: {best_match.method}"}
                )
            if api_response.get("success"):
                formatted_response = _format_api_response_for_human(
