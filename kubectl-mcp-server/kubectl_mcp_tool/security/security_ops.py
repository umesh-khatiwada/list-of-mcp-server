...
class KubernetesSecurityOps:
    ...
    def create_role(
        self, name: str, rules: List[Dict[str, Any]], namespace: str = "default"
    ) -> Dict[str, Any]:
        ...
        try:
            role = self.rbac_v1.create_namespaced_role(
                namespace=namespace,
                body=client.V1Role(
                    metadata=client.V1ObjectMeta(name=name), rules=rules
                ),
            )
            return {
                "status": "success",
            ...
            return {"status": "error", "error": f"Failed to create Role: {e.reason}"}
        ...
        try:
            role_binding = self.rbac_v1.create_namespaced_role_binding(
                ...
                body=client.V1RoleBinding(
                    ...
                    subjects=[
                        client.V1Subject(
                            kind=subject_kind, name=subject_name, namespace=namespace
                        )
                    ],
                ),
            )
        ...
        try:
            ...
            for cr in cluster_roles.items:
                results["cluster_roles"].append(
                    {"name": cr.metadata.name, "rules": cr.rules}
                )
            ...
            return {"status": "success", "audit_results": results}
        except ApiException as e:
            return {
                "status": "error",
        ...
        for container in container_specs:
            ...
            if container_security.get("privileged", False):
                ...
                recommendations.append(
                    f"Remove privileged mode from container {container['name']}"
                )
            ...
            if not container_security.get("readOnlyRootFilesystem", False):
                ...
                recommendations.append(
                    f"Set readOnlyRootFilesystem: true for container {container['name']}"
                )
            ...
            if container_security.get("allowPrivilegeEscalation", True):
                ...
                recommendations.append(
                    f"Set allowPrivilegeEscalation: false for container {container['name']}"
                )
    ...
    def analyze_network_policies(self, namespace: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            policies = (
                ...
                if namespace
                else networking_v1.list_network_policy_for_all_namespaces()
            )
            ...
            for policy in policies.items:
                ...
                if policy.spec.ingress:
                    for rule in policy.spec.ingress:
                        if not rule.from_:
                            policy_analysis["warnings"].append(
                                "Ingress rule allows traffic from all sources"
                            )
                        policy_analysis["ingress_rules"].append(
                            self._analyze_network_rule(rule)
                        )
                ...
                if policy.spec.egress:
                    for rule in policy.spec.egress:
                        ...
                        policy_analysis["egress_rules"].append(
                            self._analyze_network_rule(rule, is_ingress=False)
                        )
