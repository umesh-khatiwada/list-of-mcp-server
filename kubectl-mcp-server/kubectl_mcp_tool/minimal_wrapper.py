...
try:
    from .claude_message_framing import ClaudeMessageFramer

    logger.debug("Successfully imported ClaudeMessageFramer from relative path")
except ImportError:
    try:
        from kubectl_mcp_tool.claude_message_framing import (
            ClaudeMessageFramer,
        )
        ...
        logger.warning(
            "Could not import ClaudeMessageFramer, message framing will not be available"
        )

CLAUDE_ENVIRONMENT = (
    os.environ.get("MCP_CLIENT", "").lower() == "claude"
    or "claude" in os.environ.get("MCP_CLIENT_INFO", "").lower()
)
...
try:
    from .enhanced_json_fix import format_json_response as enhanced_format_json_response
    from .enhanced_json_fix import parse_json_safely, sanitize_json

    try:
        from .taskgroup_fix import wrap_async_handler as enhanced_wrap_async_handler

        wrap_async_handler = enhanced_wrap_async_handler
        logger.info("Imported taskgroup_fix for TaskGroup error handling")
    except ImportError:
        try:
            from .enhanced_json_fix import (
                wrap_async_handler as enhanced_wrap_async_handler,
            )
    ...
    try:
        from kubectl_mcp_tool.enhanced_json_fix import (
            format_json_response as enhanced_format_json_response,
        )
        from kubectl_mcp_tool.enhanced_json_fix import parse_json_safely, sanitize_json

        try:
            from kubectl_mcp_tool.taskgroup_fix import (
                wrap_async_handler as enhanced_wrap_async_handler,
            )
...
def run_kubectl_command(command):
    ...
    try:
        ...
        if (
            ...
            and "USE_GKE_GCLOUD_AUTH_PLUGIN" not in env
        ):
        ...
        return {"command": command, "result": f"Error: {str(e)}", "success": False}
...
def format_json_response(data: Dict[str, Any]) -> str:
    ...
    if CLAUDE_ENVIRONMENT and claude_message_framer is not None:
        try:
            ...
            logger.debug(
                f"Using ClaudeMessageFramer to frame response with ID {message_id}"
            )
        ...
        if CLAUDE_ENVIRONMENT:
            try:
                try:
                    ...
                    from .claude_json_fix_v3 import (
                        sanitize_json_for_claude as claude_sanitize_v3,
                    )

                    sanitize_json_response_func = claude_sanitize_v3
                    logger.debug(
                        "Successfully imported sanitize_json_for_claude from claude_json_fix_v3"
                    )
                except ImportError:
                    try:
                        from kubectl_mcp_tool.claude_json_fix_v3 import (
                            ensure_claude_json_safety,
                        )
                        from kubectl_mcp_tool.claude_json_fix_v3 import (
                            sanitize_json_for_claude as claude_sanitize_v3,
                        )
                        ...
                        sanitize_json_response_func = claude_sanitize_v3
                        logger.debug(
                            "Successfully imported sanitize_json_for_claude from absolute path claude_json_fix_v3"
                        )
                    except ImportError:
                        logger.warning(
                            "Could not import claude_json_fix_v3, falling back to claude_json_fix"
                        )
                        from .claude_json_fix import (
                            sanitize_json_for_claude as claude_sanitize,
                        )

                        sanitize_json_response_func = claude_sanitize
                        logger.debug(
                            "Successfully imported sanitize_json_for_claude from relative path for Claude environment"
                        )
            except ImportError:
                try:
                    from kubectl_mcp_tool.claude_json_fix import (
                        sanitize_json_for_claude as claude_sanitize,
                    )

                    sanitize_json_response_func = claude_sanitize
                    logger.debug(
                        "Successfully imported sanitize_json_for_claude from absolute path for Claude environment"
                    )
                except ImportError:
                    logger.warning(
                        "Could not import sanitize_json_for_claude from claude_json_fix, falling back to enhanced_json_fix"
                    )
                    from .enhanced_json_fix import (
                        sanitize_json_response as enhanced_sanitize,
                    )
            ...
            sanitize_json_response_func = enhanced_sanitize
            logger.debug(
                "Successfully imported sanitize_json_response from relative path"
            )
    except ImportError:
        try:
            if CLAUDE_ENVIRONMENT:
                ...
                sanitize_json_response_func = enhanced_sanitize
                logger.debug(
                    "Successfully imported sanitize_json_response from absolute path"
                )
        except ImportError:
            logger.warning(
                "Could not import sanitize_json_response from enhanced_json_fix"
            )
            ...
            try:
                ...
                if CLAUDE_ENVIRONMENT and "ensure_claude_json_safety" in locals():
                    try:
                        ...
                        logger.debug(
                            "Applied ensure_claude_json_safety for final validation"
                        )
                        return final_result
                    except Exception as e:
                ...
                logger.error(
                    f"Enhanced sanitize_json_response produced invalid JSON: {e}"
                )

                if (
                    CLAUDE_ENVIRONMENT
                    and "after JSON" in str(e)
                    and "ensure_claude_json_safety" in locals()
                ):
                    try:
                        fixed_result = ensure_claude_json_safety(result)
                        logger.debug(
                            "Fixed 'after JSON' error with ensure_claude_json_safety"
                        )
                        return fixed_result
    ...
    if enhanced_format_json_response:
        try:
            ...
            try:
                ...
                logger.error(
                    f"Enhanced format_json_response produced invalid JSON: {e}"
                )
        except Exception as e:
    ...
    try:
        ...
        json_output = json.dumps(
            sanitized_data, ensure_ascii=True, separators=(",", ":")
        )
        ...
        if clean_json and clean_json[0] == "{" and len(clean_json) > 4:
            if not clean_json[3].isalnum() and clean_json[3] not in [
                '"',
                "'",
                "_",
                ":",
                ...
                logger.warning(
                    f"Detected potential position 4 error character: '{clean_json[3]}' (Unicode: U+{ord(clean_json[3]):04X})"
                )
        ...
        try:
            ...
            if "position 4" in str(e) and len(clean_json) > 4:
                logger.warning(
                    f"Position 4 error detected: character at position 4 is '{clean_json[3]}' (Unicode: U+{ord(clean_json[3]):04X})"
                )
                fixed_json = clean_json[:3] + clean_json[4:]
                try:
                    json.loads(fixed_json)
                    logger.debug(
                        "Successfully fixed JSON by removing character at position 4"
                    )
                    clean_json = fixed_json
                except json.JSONDecodeError as e2:
                    logger.error(
                        f"Still invalid after removing character at position 4: {e2}"
                    )
            ...
            try:
                ...
                clean_json = json.dumps(
                    error_obj, ensure_ascii=True, separators=(",", ":")
                )
        ...
        if os.environ.get("MCP_DEBUG", "").lower() in ("1", "true"):
            if len(clean_json) > 10:
                ...
                for i in range(min(10, len(clean_json))):
                    logger.debug(
                        f"Char at position {i}: '{clean_json[i]}' (Unicode: U+{ord(clean_json[i]):04X})"
                    )
        ...
        try:
            ...
            return json.dumps(
                {"error": "JSON formatting error", "success": False},
                ensure_ascii=True,
                separators=(",", ":"),
            )
...
def sanitize_json_string(json_string: str) -> str:
    ...
    if sanitize_json:
        try:
            ...
            logger.error(
                f"Enhanced JSON sanitization failed: {e}, falling back to basic sanitization"
            )
    ...
    for char in problematic_chars:
        if char in json_string:
            logger.debug(
                f"Removing problematic character {repr(char)} from JSON string"
            )
...
def process_advanced_query(query: str) -> Dict[str, Any]:
    ...
    if re.search(r"(resource|cpu|memory).*usage", query) or re.search(
        r"(show|get|display).*resource", query
    ):
    ...
    if re.search(r"(show|get|display|list).*pod", query) and re.search(
        r"(resource|cpu|memory|usage)", query
    ):
        return get_pod_resource_usage()

    if re.search(r"(show|get|display|list).*node", query) and re.search(
        r"(resource|cpu|memory|usage)", query
    ):
        return get_node_resource_usage()
...
async def main():
    ...
    try:
        try:
            ...
            try:
                ...
                logger.warning(
                    "Could not import FastMCP patch, TaskGroup errors may occur"
                )
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Current PYTHONPATH: {os.environ.get('PYTHONPATH', 'Not set')}"
                )
                logger.error(f"Current sys.path: {sys.path}")
                raise ImportError(
                    "Could not import FastMCP. Please check your PYTHONPATH."
                ) from e
    ...
    async def process_natural_language(
        query: str = None, args: List[str] = None, **kwargs
    ):
        ...
        try:
            ...
            if re.search(
                r"\b(show|get|list|display)\s+(me\s+)?(all\s+)?(the\s+)?pod[s]?\b",
                query_lower,
            ):
                ...
                if re.search(
                    r"\bin\s+all\s+namespaces\b|\bacross\s+all\s+namespaces\b|\ball\s+namespaces\b",
                    query_lower,
                ):
                    namespace_arg = "--all-namespaces"
                elif re.search(
                    r"\bin\s+namespace\b|\bin\s+the\s+namespace\b|\bin\s+[\w-]+\s+namespace\b",
                    query_lower,
                ):
                    namespace_match = re.search(
                        r"in\s+(?:the\s+)?(?:namespace\s+)?([\w-]+)(?:\s+namespace)?",
                        query_lower,
                    )
                    if namespace_match:
            ...
            elif re.search(r"\bnamespace[s]?\b", clean_query) and (
                "list" in clean_query or clean_query.strip() == "namespaces"
            ):
            ...
            try:
                ...
                try:
                    try:
                        from kubectl_mcp_tool.taskgroup_fix import (
                            run_with_taskgroup_protection,
                        )

                        result = await run_with_taskgroup_protection(
                            asyncio.to_thread(process_query, query, args, **kwargs)
                        )
                    except ImportError:
                        result = await asyncio.to_thread(
                            process_query, query, args, **kwargs
                        )

                    if (
                        isinstance(result, dict)
                        and "jsonrpc" in result
                        and "error" in result
                    ):
                        logger.info(
                            f"Received error response from TaskGroup protection: {result}"
                        )
                        json_result = format_and_validate_json_response(result)
                        logger.debug(
                            f"Formatted JSON result: {repr(json_result[:20])}..."
                        )
                        return json_result
                    ...
                    if "unhandled errors in a TaskGroup" in str(e):
                        logger.error(
                            f"TaskGroup error in natural language processing: {e}"
                        )
                        error_result = {
                            "jsonrpc": "2.0",
                            "id": None,
    ...
    @wrap_async_handler
    async def advanced_kubernetes_query(
        query: str = None, args: List[str] = None, **kwargs
    ):
    ...
    try:
        ...
        if "unhandled errors in a TaskGroup" in str(e):
            logger.error(
                "Detected TaskGroup error, providing structured error response"
            )
            error_result = {
                "jsonrpc": "2.0",
                "id": None,
                "error": {
                    "code": -32603,
                    "message": "Internal error in TaskGroup",
                    "data": {"error_type": "taskgroup_error", "original_error": str(e)},
                },
            }
            print(format_and_validate_json_response(error_result))
    finally:
    ...
    try:
        ...
        try:
            ...
            if "position 4" in str(e) and len(json_result) > 4:
                logger.warning(
                    f"Position 4 error detected: '{json_result[3]}' (Unicode: U+{ord(json_result[3]):04X})"
                )
