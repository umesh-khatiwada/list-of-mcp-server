        ...
        if match:
            ...
            if timestamp_start == 0 and "{" in json_string[timestamp_end:]:
                ...
                if start < end:
                    ...
                    logger.debug(
                        f"Extracted JSON content after timestamp: {json_string[:30]}..."
                    )
            elif timestamp_start > 0:
                before = json_string[:timestamp_start]
                after = json_string[timestamp_end:]

                if before.count("{") > before.count("}") and after.count(
                    "}"
                ) > after.count("{"):
                    json_string = before + after
                    logger.debug(
                        f"Removed timestamp from middle of JSON: {json_string[:30]}..."
                    )
    ...
    async def _read_message(self, stream: Any) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if os.environ.get("MCP_DEBUG", "").lower() in ("1", "true"):
                ...
                if len(line_str) > 10:
                    for i in range(min(10, len(line_str))):
                        logger.debug(
                            f"Char at position {i}: '{line_str[i]}' (Unicode: U+{ord(line_str[i]):04X})"
                        )
            ...
            for pattern in timestamp_patterns:
                ...
                if match:
                    logger.debug(
                        f"Detected log line with timestamp using pattern {pattern}, attempting to extract JSON"
                    )
                    timestamp_start = match.start(1)
                    timestamp_end = match.end(1)
                    ...
                    if timestamp_start == 0:
                        if "{" in line_str[timestamp_end:]:
                            ...
                            if start < end:
                                ...
                                logger.debug(
                                    f"Extracted JSON after timestamp: {line_str[:30]}..."
                                )
                    else:
                        ...
                        if before.count("{") > before.count("}") and after.count(
                            ...
                            logger.debug(
                                f"Removed timestamp from middle of JSON: {line_str[:30]}..."
                            )
            ...
            if parse_json_safely:
                ...
                if parsed:
                    logger.debug(
                        "Successfully parsed message using enhanced JSON parser"
                    )
                    return parsed
                ...
                if "{" in sanitized and "}" in sanitized:
                    ...
                    if start < end:
                        ...
                        try:
                            ...
                            if os.environ.get("MCP_DEBUG", "").lower() in ("1", "true"):
                                logger.debug(
                                    f"Failed JSON content: {potential_json[:100]}..."
                                )
    ...
    async def _write_message(self, message: Dict[str, Any], stream: Any) -> None:
        ...
        try:
            ...
            for pattern in timestamp_patterns:
                ...
                if match:
                    logger.warning(
                        f"Found timestamp with pattern {pattern} in JSON string"
                    )
                    timestamp_start = match.start(1)
                    timestamp_end = match.end(1)
                    ...
                    if timestamp_start == 0 and "{" in json_str[timestamp_end:]:
                        ...
                        if start < end:
                            ...
                            logger.debug(
                                f"Extracted JSON content after timestamp: {json_str[:30]}..."
                            )
                    elif timestamp_start > 0:
                        ...
                        if before.count("{") > before.count("}") and after.count(
                            ...
                            logger.debug(
                                f"Removed timestamp from middle of JSON: {json_str[:30]}..."
                            )
            ...
            try:
                ...
                for pattern in timestamp_patterns:
                    ...
                    if match:
                        logger.warning(
                            f"Found timestamp pattern: {pattern} in JSON, attempting to extract clean JSON"
                        )
                        timestamp_start = match.start(1)
                        timestamp_end = match.end(1)
                        ...
                        if timestamp_start == 0:
                            if "{" in clean_json[timestamp_end:]:
                                ...
                                if start < end:
                                    ...
                                    logger.debug(
                                        f"Extracted JSON after timestamp: {clean_json[:30]}..."
                                    )
                                    break
                        else:
                            ...
                            if before.count("{") > before.count("}") and after.count(
                                ...
                                logger.debug(
                                    f"Removed timestamp from middle of JSON: {clean_json[:30]}..."
                                )
                                break
