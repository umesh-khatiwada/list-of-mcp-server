...
try:
    ...
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "mcp>=1.5.0"])
        from mcp.server.fastmcp import FastMCP
...
class MCPServer:
    ...
    def __init__(self, name: str):
        ...
        if not self.dependencies_available:
            logger.warning(
                "Some dependencies are missing. Certain operations may not work correctly."
            )
    ...
    def setup_tools(self):
        ...
        def install_helm_chart(
            ...
            if not self._check_helm_availability():
                return {
                    "success": False,
                    "error": "Helm is not available on this system",
                }
            ...
            try:
                ...
                if repo:
                    try:
                        ...
                        if len(repo_parts) != 2:
                            return {
                                "success": False,
                                "error": "Repository format should be 'repo_name=repo_url'",
                            }
                        ...
                        subprocess.check_output(
                            repo_add_cmd, stderr=subprocess.PIPE, text=True
                        )
                        ...
                        subprocess.check_output(
                            repo_update_cmd, stderr=subprocess.PIPE, text=True
                        )
                        ...
                        logger.error(
                            f"Error adding Helm repo: {e.stderr if hasattr(e, 'stderr') else str(e)}"
                        )
                        return {
                            "success": False,
                            "error": f"Failed to add Helm repo: {e.stderr if hasattr(e, 'stderr') else str(e)}",
                        }
                ...
                try:
                    ...
                    try:
                        subprocess.check_output(
                            create_ns_cmd, stderr=subprocess.PIPE, text=True
                        )
                    except subprocess.CalledProcessError as e:
                        logger.error(
                            f"Error creating namespace: {e.stderr if hasattr(e, 'stderr') else str(e)}"
                        )
                        return {
                            "success": False,
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Failed to install Helm chart: {error_msg}",
                    }
                finally:
        ...
        def upgrade_helm_chart(
            ...
            try:
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Failed to upgrade Helm chart: {error_msg}",
                    }
                finally:
                    # Clean up the temporary values file
                    if values_file and os.path.exists(values_file):
        ...
        def uninstall_helm_chart(name: str, namespace: str) -> Dict[str, Any]:
            ...
            try:
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Failed to uninstall Helm chart: {error_msg}",
                    }
            except Exception as e:
            ...
            try:
                ...
                try:
                    pod_output = subprocess.check_output(
                        pod_cmd, stderr=subprocess.PIPE, text=True
                    )
                ...
                try:
                    ...
                    node_output = subprocess.check_output(
                        node_cmd, stderr=subprocess.PIPE, text=True
                    )
                ...
                return {"success": True, "pod_usage": pod_data, "node_usage": node_data}
            except subprocess.CalledProcessError as e:
                error_msg = e.stderr if hasattr(e, "stderr") else str(e)
                logger.error(f"Error getting resource usage: {error_msg}")
                return {
                    "success": False,
                    "error": f"Failed to get resource usage: {error_msg}",
                }
            except Exception as e:
        ...
        def switch_context(context_name: str) -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    "success": True,
                    "message": f"Switched context to {context_name}",
                }
            except Exception as e:
        ...
        def check_pod_health(
            ...
            try:
                ...
                return {
                    "success": True,
                    "phase": status.phase,
                    "conditions": [c.type for c in status.conditions]
        ...
        @self.server.tool()
        def delete_resource(
            resource_type: str, name: str, namespace: Optional[str] = "default"
        ) -> Dict[str, Any]:
        ...
        def scale_deployment(
            name: str, replicas: int, namespace: Optional[str] = "default"
        ) -> Dict[str, Any]:
            """Scale a deployment."""
            try:
                from kubernetes import client, config
    ...
    def _check_dependencies(self) -> bool:
        ...
        for tool in ["kubectl", "helm"]:
            if not self._check_tool_availability(tool):
                logger.warning(
                    f"{tool} not found in PATH. Operations requiring {tool} will not work."
                )
                all_available = False
    ...
    def _check_tool_availability(self, tool: str) -> bool:
        ...
        try:
            ...
            if tool == "kubectl":
                subprocess.check_output(
                    [tool, "version", "--client"], stderr=subprocess.PIPE
                )
            elif tool == "helm":
    ...
    async def serve_stdio(self):
        ...
        logger.info(
            f"Dependencies check result: {'All available' if self.dependencies_available else 'Some missing'}"
        )
