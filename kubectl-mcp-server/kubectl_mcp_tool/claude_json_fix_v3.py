...
def extract_clean_json(text: str) -> str:
    ...
    try:
        ...
        logger.debug(
            f"Successfully extracted and cleaned JSON: {repr(clean_json[:50])}..."
        )
        return clean_json
    ...
    try:
        ...
        try:
            ...
            if "after JSON" in str(e):
                logger.warning(
                    "Detected 'after JSON' error, attempting to extract clean JSON"
                )
...
def ensure_claude_json_safety(json_str: str) -> str:
    ...
    try:
        ...
        if len(final_json) > 4:
            if not final_json[3].isalnum() and final_json[3] not in [
                '"',
                "'",
                "_",
                ":",
                ...
                logger.warning(
                    f"Final check: Detected position 4 error character: '{final_json[3]}' (Unicode: U+{ord(final_json[3]):04X})"
                )
