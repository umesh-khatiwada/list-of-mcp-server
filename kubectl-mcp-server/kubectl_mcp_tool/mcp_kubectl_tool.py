...
def _run_kubectl_command(command: List[str]) -> str:
    ...
    try:
        result = subprocess.run(
            ["kubectl"] + command, capture_output=True, text=True, check=True
        )
...
def format_kubectl_output(output: str, resource_type: str = "") -> str:
                ...
                if line.strip():  # Skip empty lines
                    ...
                    for header in headers:
                        ...
                        if header == headers[-1]:  # Last column takes rest of line
                            ...
                            while (
                                split_point < len(current_line)
                                and current_line[split_point].isspace()
                            ):
                                split_point += 1
    ...
    if "created" in output or "configured" in output:
        ...
        with console.capture() as capture:
            console.print(
                Panel(styled_output, title="Resource Deleted", border_style="yellow")
            )
        return capture.get()
        ...
        with console.capture() as capture:
            console.print(
                Panel(styled_output, title="Scaling Complete", border_style="blue")
            )
        return capture.get()
...
# Tool handlers
async def get_pods(
    namespace: str = "", pod_name: str = "", output_format: str = "wide"
) -> str:
...
async def create_pod(pod_yaml: str, namespace: str = "") -> str:
...
async def delete_pod(pod_name: str, namespace: str = "") -> str:
...
async def scale_deployment(
    deployment_name: str, replicas: int = 1, namespace: str = ""
) -> str:
...
async def get_namespaces(namespace_name: str = "", output_format: str = "wide") -> str:
...
async def create_namespace(namespace_name: str) -> str:
    """Create a namespace."""
...
async def delete_namespace(namespace_name: str) -> str:
    """Delete a namespace."""
...
def main(port: int, transport: str) -> int:
    ...
    async def get_namespaces_tool(
        namespace_name: str = "", output_format: str = "wide"
    ) -> list[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Get information about namespaces.
    ...
    async def get_resources_tool(
        ...
        result = await get_resources(
            resource_type, resource_name, namespace, output_format
        )
    ...
    logger.info(
        f"Starting kubectl MCP server with {transport} transport on port {port}"
    )
    if transport == "stdio":
