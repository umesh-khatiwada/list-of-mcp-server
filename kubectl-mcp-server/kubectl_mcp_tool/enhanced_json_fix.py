...
def extract_json_from_log(text: str) -> Optional[str]:
    ...
    if text.startswith("202") and " - INFO - Starting MCP server" in text:
        ...
        standard_response = {
            ...
            "id": 0,
            "result": {
                "serverInfo": {"name": "kubectl-mcp-tool", "version": "1.1.0"},
    ...
    timestamp_pattern = (
        r"^\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}:\d{2}[,\.]\d{3}\s+-\s+\w+\s+-\s+(.*)"
    )
                    ...
                    for i in range(start + 1, min(start + 20, len(text))):
                        if i < len(text) and text[i] == "{":
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Found valid JSON starting at position {i}"
                                )
    ...
    if len(text) > 5 and text[4] == "-" and text.startswith("202"):
        logger.debug(
            "Detected timestamp at start, attempting to extract JSON after timestamp"
        )

        if " - INFO - Starting MCP server" in text:
            logger.debug(
                "Detected MCP server startup message, creating standard response"
            )
            standard_response = {
                "jsonrpc": "2.0",
                "id": 0,
            ...
            if i < len(text) and text[i] == "{":
                ...
                if i < end_pos:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Extracted JSON after timestamp starting at position {i}"
                        )
...
def sanitize_json(text: str) -> Optional[str]:
    ...
    if len(text) > 4:
        if text[4] in ["-", "\ufeff", "\u200b", "\u200c", "\u200d", "\u2060"]:
            logger.debug(
                f"Detected problematic character at position 4: {repr(text[4])}"
            )
            ...
            if text.startswith("202") and text[4] == "-":
                ...
                try:
                    ...
                    logger.debug(
                        f"Successfully fixed JSON by removing character at position 4"
                    )
                    return fixed_text
                except json.JSONDecodeError:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Successfully fixed and sanitized JSON by removing character at position 4"
                        )
        ...
        if pattern in text:
            ...
            return {
                ...
                "error": {
                    ...
                    "data": {"original_error": text, "error_type": "connection_error"},
                },
            }
                ...
                if "error" in parsed:
                    return {
                        ...
                        "error": {
                            ...
                            "data": {
                                "original_error": text.split(
                                    "Error running server:", 1
                                )[1].strip(),
        ...
        if "unhandled errors in a TaskGroup" in text:
            ...
            return {
                ...
                "error": {
                    ...
                    "data": {
                        "original_error": text.split("Error running server:", 1)[
                            1
                        ].strip(),
...
def sanitize_json_response(data: Union[Dict[str, Any], list]) -> str:
    ...
    try:
        ...
        if json_str and json_str[0] == "{" and len(json_str) > 4:
            if not json_str[3].isalnum() and json_str[3] not in [
                '"',
                "'",
                "_",
                ":",
                ...
                logger.warning(
                    f"Detected potential position 4 error character: '{json_str[3]}' (Unicode: U+{ord(json_str[3]):04X})"
                )
        ...
        try:
            ...
            if "position 4" in str(e) and len(json_str) > 4:
                logger.warning(
                    f"Position 4 error detected: character at position 4 is '{json_str[3]}' (Unicode: U+{ord(json_str[3]):04X})"
                )
                    ...
                    for i in range(5, 10):
                        if len(json_str) > i:
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Successfully fixed JSON by removing characters 4 through {i}"
                                )
        ...
        if json_str.startswith("202") and len(json_str) > 4 and json_str[4] == "-":
            ...
            for i in range(20, len(json_str)):
                if i < len(json_str) and json_str[i] == "{":
                    json_str = json_str[i:]
                    logger.debug(
                        f"Removed timestamp prefix, JSON now starts with: {json_str[:20]}..."
                    )
                    break
