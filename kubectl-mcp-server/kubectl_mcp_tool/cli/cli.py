...
def main():
    ...
    serve_parser.add_argument(
        "--port", type=int, default=8080, help="Port to use for SSE transport"
    )
    serve_parser.add_argument(
        "--cursor", action="store_true", help="Enable Cursor compatibility mode"
    )
    serve_parser.add_argument(
        ...
        if args.command == "serve":
            if args.cursor:
                try:
                    ...
                    logger.error(
                        "Make sure cursor_mcp_server.py is in the correct location"
                    )
                    exit_code = 1
                except Exception as e:
