    ...
    try:
        ...
        if len(result) > 10:
            ...
            for i in range(min(10, len(result))):
                logger.debug(
                    f"Char at position {i}: '{result[i]}' (Unicode: U+{ord(result[i]):04X})"
                )
...
def claude_json_encode(obj: Any) -> str:
    ...
    try:
        ...
        json_str = json.dumps(
            obj, cls=ClaudeJSONEncoder, ensure_ascii=True, separators=(",", ":")
        )
        ...
        return json.dumps(
            {"error": str(e), "success": False, "value": str(obj)},
            ensure_ascii=True,
            separators=(",", ":"),
        )
...
def sanitize_json_string(json_string: str) -> str:
    ...
    if json_string and len(json_string) > 4:
        if not json_string[3].isalnum() and json_string[3] not in [
            '"',
            "'",
            "_",
            ":",
            ...
            logger.warning(
                f"Detected potential position 4 error character: '{json_string[3]}' (Unicode: U+{ord(json_string[3]):04X})"
            )
    ...
    try:
        ...
        if "position 4" in str(e) and len(json_string) > 4:
            logger.warning(
                f"Position 4 error detected: character at position 4 is '{json_string[3]}' (Unicode: U+{ord(json_string[3]):04X})"
            )
