...
def _run_kubectl_command(args: List[str]) -> Tuple[str, bool]:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(
                f"kubectl command failed with exit code {result.returncode}: {result.stderr}"
            )
...
def process_query(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get pods command
        if (
            "get all pods" in query_lower
            or "get pods" in query_lower
            or "show pods" in query_lower
            or "list pods" in query_lower
        ):
            namespace_arg = []
            namespace = None
            ...
            if "in namespace" in query_lower:
                ...
                "in all namespaces" in query_lower
                or "across all namespaces" in query_lower
            ):
            ...
            return {"command": "kubectl " + " ".join(cmd), "result": result}

        # Get namespaces command
        elif (
            "show namespaces" in query_lower
            or "get namespaces" in query_lower
            or "list namespaces" in query_lower
        ):
            cmd = ["get", "namespaces"]
            result, success = _run_kubectl_command(cmd)
        ...
        # Switch namespace command
            ...
            "switch to namespace" in query_lower
            or "change namespace" in query_lower
            or "use namespace" in query_lower
        ):
        ...
        # Get deployments command
            ...
            "get deployments" in query_lower
            or "show deployments" in query_lower
            or "list deployments" in query_lower
        ):
            namespace_arg = []
            namespace = None
        ...
        # Get services command
            ...
            "get services" in query_lower
            or "show services" in query_lower
            or "list services" in query_lower
        ):
            namespace_arg = []
            namespace = None
        ...
        # For other resource types
            ...
            for resource in [
                ...
                if (
                    f"get {resource}" in query_lower
                    or f"show {resource}" in query_lower
                    or f"list {resource}" in query_lower
                ):
                    namespace_arg = []
                    namespace = None
        ...
        return {"command": "Error", "result": f"Error processing query: {str(e)}"}
