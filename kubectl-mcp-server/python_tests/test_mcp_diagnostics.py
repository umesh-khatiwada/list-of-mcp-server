...
class TestMCPDiagnostics:
    ...
    def temp_resource_file(self, resource_dict, filename_suffix=""):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=f"{filename_suffix}.json", delete=False
        ) as temp:
    ...
    def test_validate_yaml(self, mcp_client):
        ...
        valid_pod = {
            ...
            "metadata": {
                "name": "valid-pod",
                "labels": {"app": "test-pod", "created-by": "mcp-test"},
            },
            "spec": {
                "containers": [
                    {
        ...
        with self.temp_resource_file(valid_pod) as pod_file:
            response = mcp_client.call_tool("validate_resource", {"filepath": pod_file})
    ...
    def test_validate_invalid_yaml(self, mcp_client):
        ...
        with self.temp_resource_file(invalid_pod, "_invalid_resource") as pod_file:
            ...
            logger.info(
                f"Validation found {len(result['errors'])} errors in invalid resource"
            )
    ...
    def test_analyze_resource_issues(self, mcp_client):
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(problematic_pod) as pod_file:
                ...
                analyze_response = mcp_client.call_tool(
                    "analyze_resource_issues",
                    {
                        "namespace": namespace,
                        "resource_type": "pod",
                        "name": "problematic-pod",
    ...
    def test_probe_validation(self, mcp_client):
        ...
        with namespace_context() as namespace:
            ...
            probe_pod = {
                ...
                "metadata": {
                    "name": "probe-test-pod",
                    "labels": {"app": "probe-test", "created-by": "mcp-test"},
                },
                "spec": {
                    "containers": [
                        {
            ...
            with self.temp_resource_file(probe_pod) as pod_file:
                ...
                # Validate the probes
                probe_response = mcp_client.call_tool(
                    "validate_probes",
                    {"namespace": namespace, "name": "probe-test-pod"},
                )
                ...
                # Check response contents
                assert (
                    probe_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in probe_response
                ), "Response should contain 'result' field"
                ...
                logger.info(
                    f"Readiness: {result['readiness'].get('status', 'Unknown')}"
                )
    ...
    def test_diagnostic_information(self, mcp_client):
        ...
        logger.info(
            f"Cluster info: {result.get('cluster_info', {}).get('name', 'Unknown')}"
        )
        logger.info(
            f"Version: {result.get('version', {}).get('gitVersion', 'Unknown')}"
        )
