...
def test_new_methods():
    ...
    try:
        ...
        logger.info(
            f"Found {result.get('count', 0)} deployments in kube-system namespace"
        )
        ...
        if result.get("count", 0) > 0 and k8s_ops.list_pods().get("count", 0) > 0:
            ...
            namespace = (
                k8s_ops.list_pods().get("items", [{}])[0].get("namespace", "default")
            )
        ...
        for method_name in methods_to_check:
            if hasattr(k8s_ops, method_name) and callable(
                getattr(k8s_ops, method_name)
            ):
