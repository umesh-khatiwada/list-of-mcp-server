...
try:
    result = subprocess.run(
        ["kubectl", "version", "--client"], capture_output=True, check=True, text=True
    )
...
try:
    result = subprocess.run(
        ["kubectl", "get", "namespaces"], capture_output=True, check=True, text=True
    )
...
try:
    pod_manifest = {
        "apiVersion": "v1",
        "kind": "Pod",
        "metadata": {"name": "test-pod-mcp", "namespace": "default"},
        "spec": {
            "containers": [
                {
                    "name": "nginx",
...
try:
    ...
    print(
        f"Pod deletion result: {result.stdout.strip() if result.returncode == 0 else result.stderr.strip()}"
    )
except Exception as e:
