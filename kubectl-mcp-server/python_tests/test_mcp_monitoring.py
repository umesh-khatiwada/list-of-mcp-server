    ...
    def test_get_events(self, mcp_client):
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(pod_spec) as pod_file:
                ...
                # Get events
                events_response = mcp_client.call_tool(
                ...
                assert (
                    events_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in events_response
                ...
                assert (
                    "items" in result
                ), "Result should contain 'items' field with events"
    ...
    def test_resource_utilization(self, mcp_client):
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(deployment_spec) as deployment_file:
                ...
                # Get pod usage
                usage_response = mcp_client.call_tool(
                    "get_resource_usage",
                ...
                # Check response contents
                assert (
                    usage_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in usage_response
                ), "Response should contain 'result' field"
                ...
                assert (
                    "items" in result
                ), "Result should contain 'items' field with usage data"

                # Log usage data
                logger.info(f"Resource usage data: {len(result['items'])} items")
                for item in result.get("items", []):
                    logger.info(
                        f"Pod: {item.get('name', 'Unknown')}, "
        ...
        with namespace_context() as namespace:
            ...
            pod_spec = {
                ...
                "metadata": {
                    ...
                    "labels": {"app": "health-test", "created-by": "mcp-test"},
                },
                "spec": {
                    "containers": [
                        {
            ...
            with self.temp_resource_file(pod_spec) as pod_file:
                ...
                # Check pod health
                health_response = mcp_client.call_tool(
                    "check_pod_health",
                    {"namespace": namespace, "name": "health-test-pod"},
                )
                ...
                # Check response contents
                assert (
                    health_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in health_response
                ), "Response should contain 'result' field"
                ...
                logger.info(
                    f"Readiness probe: {result.get('probes', {}).get('readiness', 'N/A')}"
                )
                logger.info(
                    f"Liveness probe: {result.get('probes', {}).get('liveness', 'N/A')}"
                )
