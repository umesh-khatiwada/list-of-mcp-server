...
def setup_test_namespace(prefix="mcp-test-", max_retries=5, wait_time=3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"Creating namespace {namespace_name} (attempt {attempt+1}/{max_retries})"
            )
            ...
            if attempt < max_retries - 1:
                ...
                raise RuntimeError(
                    f"Failed to create namespace after {max_retries} attempts"
                )
...
def generate_test_pod(name: str, image: str = "nginx:latest") -> Dict[str, Any]:
    ...
    return {
        "apiVersion": "v1",
        "kind": "Pod",
        "metadata": {"name": name, "labels": {"app": name, "created-by": "mcp-test"}},
        "spec": {
            "containers": [
                {"name": "main", "image": image, "ports": [{"containerPort": 80}]}
            ]
        },
    }
...
def generate_test_deployment(
    name: str, replicas: int = 1, image: str = "nginx:latest"
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "spec": {
            ...
            "template": {
                "metadata": {"labels": {"app": name, "created-by": "mcp-test"}},
                "spec": {
                    "containers": [
                        {
                            "name": "main",
...
def validate_mcp_response(
    response: Dict[str, Any], schema: Optional[Dict[str, Any]] = None
) -> Tuple[bool, str]:
    """
    ...
    if response["type"] not in valid_types:
        return (
            False,
            f"Expected response type to be one of {valid_types}, got '{response['type']}'",
        )
    ...
    if schema and response["type"] == "tool_call":
        ...
        for key, expected_type in schema.items():
            ...
            if not isinstance(response["result"][key], expected_type):
                return (
                    False,
                    f"Field '{key}' has wrong type: expected {expected_type}, got {type(response['result'][key])}",
                )

    return True, ""


def validate_kubernetes_state(
    ...
    if MOCK_MODE:
        ...
        logger.info(
            f"[MOCK] Validating {resource_type}/{name} in namespace {namespace}"
        )
        return True, {
