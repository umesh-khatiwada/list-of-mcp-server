...
class TestKubectlMcp:
    ...
    def send_request(
        self, method: str, params: dict = None, timeout: int = 5
    ) -> Optional[dict]:
        ...
        try:
            ...
            while time.time() - start_time < timeout:  # Use provided timeout
                ...
                )  # No need to decode for text mode
                if not line:
                    time.sleep(0.1)
                    continue
            ...
            logger.error(
                f"Timeout after {elapsed:.2f}s waiting for response to {method}"
            )
            if method == "tools/call" and params:
                logger.error(
                    f"Tool: {params.get('tool')}, params: {params.get('params')}"
                )
    ...
    def test_initialization(self):
        ...
        assert (
            result["name"] == "kubectl-mcp-server-simple"
        ), f"Unexpected server name: {result['name']}"
    ...
    def test_pod_management(self):
        ...
        pod_spec = {
            "apiVersion": "v1",
            "kind": "Pod",
            "metadata": {"name": "test-pod", "namespace": self.test_namespace},
        ...
        response = self.send_request(
            ...
            {
                "tool": "create_pod",
                "params": {"pod_spec": pod_spec, "namespace": self.test_namespace},
            },
        )
        ...
        response = self.send_request(
            "tools/call",
            {
                ...
                "params": {"pod_name": "test-pod", "namespace": self.test_namespace},
            },
        )
        if not response or "result" not in response:
    ...
    def test_deployment_management(self):
        ...
        deployment_spec = {
            "apiVersion": "apps/v1",
            "kind": "Deployment",
            "metadata": {"name": "test-deployment", "namespace": self.test_namespace},
            "spec": {
                "replicas": 2,
    ...
    def test_security_features(self):
        ...
        response = self.send_request(
            "tools/call",
            {
                ...
                "params": {"name": "test-sa", "namespace": self.test_namespace},
            },
        )
        if not response or "result" not in response:
        ...
        response = self.send_request(
            ...
            {
                "tool": "audit_rbac",
                "params": {"namespace": self.test_namespace, "audit_type": "roles"},
            },
        )
        if not response or "result" not in response:
        ...
        if result["status"] != "success":
            raise AssertionError(
                f"Roles audit failed: {result.get('message', 'No error message')}"
            )
        ...
        if result["status"] != "success":
            raise AssertionError(
                f"Role bindings audit failed: {result.get('message', 'No error message')}"
            )
        audit_results.update(result["audit_results"])
        ...
        for chunk_attempt in range(max_chunks):
            ...
            while retry_count < max_retries and not success:
                try:
                    logger.info(
                        f"Processing cluster roles chunk {chunk_number} (attempt {retry_count+1})"
                    )
                    response = self.send_request(
                        "tools/call",
                        {
                        ...
                        timeout=90,
                    )  # Extra long timeout for this specific call

                    if not response or "result" not in response:
                        logger.error(
                            f"No response received from cluster roles audit chunk {chunk_number}"
                        )
                        retry_count += 1
                    ...
                    if result["status"] != "success":
                        logger.error(
                            f"Cluster roles audit chunk {chunk_number} failed: {result.get('message', 'No error message')}"
                        )
                        retry_count += 1
                        time.sleep(2)
                        continue
                    ...
                    logger.info(
                        f"Processed cluster roles chunk {current_chunk}/{total_chunks}, progress: {progress:.1f}%"
                    )
                    ...
                    logger.error(
                        f"Timeout processing cluster roles chunk {chunk_number}: {e}"
                    )
                    retry_count += 1
                    time.sleep(3)  # Wait even longer after timeout
                except Exception as e:
                    logger.error(
                        f"Error processing cluster roles chunk {chunk_number}: {e}"
                    )
                    retry_count += 1
                    time.sleep(2)
            ...
            if not success:
                logger.warning(
                    f"Giving up on chunk {chunk_number} after {max_retries} failed attempts"
                )
                break
        ...
        logger.info(
            f"Processed {len(cluster_roles)} cluster roles in {chunk_number + 1} chunks"
        )

        # Skip cluster role bindings if we got no data from cluster roles
        if len(cluster_roles) > 0:
            logger.info(
                "At least some cluster roles processed successfully, continuing with test"
            )
        else:
        ...
        if not test_role:
            ...
            logger.warning(
                "Test role not found in audit results, but continuing with test"
            )
        else:
    ...
    def test_monitoring_features(self):
        ...
        response = self.send_request(
            "tools/call",
            {
                ...
                "params": {"namespace": self.test_namespace},
            },
        )
        assert response["result"]["status"] == "success"
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Cleanup of existing namespace failed (this is usually ok): {e}"
                )
            ...
            for i in range(max_retries):
                try:
                    ...
                    if i < max_retries - 1:
                        logger.warning(
                            f"Failed to create namespace (attempt {i+1}/{max_retries}): {e}"
                        )
                        ...
                        logger.error(
                            f"Failed to create namespace after {max_retries} attempts: {e}"
                        )
                        raise
