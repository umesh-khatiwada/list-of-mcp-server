...
async def simulate_mcp_conversation(proc):
    ...
    # Wait for response
    list_tools_response = await asyncio.get_event_loop().run_in_executor(
        None, proc.stdout.readline
    )
    ...
    try:
        ...
        if "result" in tools_resp and "tools" in tools_resp["result"]:
            print(
                f"Available tools: {[tool['name'] for tool in tools_resp['result']['tools']]}"
            )
        else:
    ...
    # Wait for response
    call_tool_response = await asyncio.get_event_loop().run_in_executor(
        None, proc.stdout.readline
    )
    ...
    call_tool_request = {
        ...
        "id": "4",
        "method": "tools/call",
        "params": {"name": "get_namespaces", "arguments": {}},
    }
