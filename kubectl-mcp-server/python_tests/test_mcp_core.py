...
class TestMCPCore:
    ...
    def test_set_and_get_namespace(self, mcp_client):
        ...
        set_response = mcp_client.call_tool(
            "set_namespace", {"namespace": "kube-system"}
        )
        ...
        # Check response contents
        assert (
            set_response["type"] == "tool_call"
        ), "Response type should be 'tool_call'"
        assert "result" in set_response, "Response should contain 'result' field"
        assert (
            set_response["result"]["success"] is True
        ), "Setting namespace should succeed"
        ...
        # Check response contents
        assert (
            get_response["result"]["namespace"] == "kube-system"
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(pod_spec) as pod_file:
                create_response = mcp_client.call_tool(
                    "create_resource", {"filepath": pod_file}
                )
                ...
                # Check response contents
                assert (
                    get_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in get_response
                ), "Response should contain 'result' field"
                ...
                assert (
                    "items" in result
                ), "Result should contain 'items' field with pods"
                assert (
                    len(result["items"]) == 1
                ), f"Expected 1 pod, got {len(result['items'])}"
                assert (
                    result["items"][0]["metadata"]["name"] == pod_name
                ...
                assert (
                    state["metadata"]["name"] == pod_name
                ), f"Expected pod name to be '{pod_name}'"
    ...
    def test_create_and_scale_deployment(self, mcp_client):
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(deployment_spec) as deployment_file:
                create_response = mcp_client.call_tool(
                    "create_resource", {"filepath": deployment_file}
                )
                ...
                # Scale deployment
                scale_response = mcp_client.call_tool(
                    "scale_deployment",
                    {
                ...
                # Check response contents
                assert (
                    scale_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in scale_response
                ), "Response should contain 'result' field"
                assert (
                    scale_response["result"]["success"] is True
                ...
                # Get deployment
                get_response = mcp_client.call_tool(
                    "get_deployments", {"namespace": namespace, "name": deployment_name}
                )
                ...
                assert (
                    "items" in result
                ), "Result should contain 'items' field with deployments"
                assert (
                    len(result["items"]) == 1
                ), f"Expected 1 deployment, got {len(result['items'])}"

                replicas = result["items"][0]["spec"]["replicas"]
                assert replicas == 2, f"Expected 2 replicas, got {replicas}"

                logger.info(
                    f"Successfully created and scaled deployment {deployment_name}"
                )

    def test_explain_resource(self, mcp_client):
        """Test explaining a Kubernetes resource."""
        response = mcp_client.call_tool("explain_resource", {"resource": "pod"})
        ...
        assert (
            "pod" in result["explanation"].lower()
        ), "Explanation should describe pods"
