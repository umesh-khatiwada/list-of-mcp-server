...
from test_utils import validate_mcp_response
...
class TestMCPNLP:
    ...
    def test_simple_query(self, mcp_client):
        ...
        response = mcp_client.call_tool("process_natural_language", {"query": query})
        ...
        assert (
            "kubernetes_command" in result
        ), "Result should contain 'kubernetes_command' field"
        assert "response" in result, "Result should contain 'response' field"

        # Verify the intent matches listing pods
        assert (
            "list" in result["intent"].lower()
        ), f"Expected list intent, got '{result['intent']}'"
        assert (
            "pod" in result["intent"].lower()
        ), f"Expected pod resource, got '{result['intent']}'"

        # Verify the command includes kubectl get pods
        assert (
            "kubectl get pods" in result["kubernetes_command"]
    ...
    def test_complex_query(self, mcp_client):
        ...
        assert (
            "deployment" in result["intent"].lower()
    ...
    def test_context_aware_query(self, mcp_client):
        ...
        assert (
            command_has_namespace or response_has_namespace
    ...
    def test_error_query(self, mcp_client):
        ...
        logger.info(f"NLP Query: '{query}'")
        logger.info(
            f"Response: {result.get('response') or result.get('error') or result.get('message')}"
        )
    ...
    def test_mock_data_query(self, mcp_client):
        ...
        response = mcp_client.call_tool(
            "process_natural_language", {"query": query, "use_mock_data": True}
        )
    ...
    def test_explanation_query(self, mcp_client):
        ...
        found_terms = [
            term for term in explanation_terms if term in result["response"].lower()
        ]
