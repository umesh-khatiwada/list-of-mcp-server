...
def check_dependencies(mock_mode=False):
    ...
    required_commands = (
        ["python"] if mock_mode else ["python"]
    ...
    if not mock_mode:
        ...
        logger.info(
            "Running in mock mode - skipping kubectl cluster connectivity check"
        )
...
def main():
    ...
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Run in mock mode without requiring Kubernetes cluster",
    )
