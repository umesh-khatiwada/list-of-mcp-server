...
class TestMCPSecurity:
    ...
    def generate_service_account(self, name):
        ...
        return {
            "apiVersion": "v1",
            "kind": "ServiceAccount",
            "metadata": {"name": name, "labels": {"created-by": "mcp-test"}},
        }
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(sa_spec) as sa_file:
                create_response = mcp_client.call_tool(
                    "create_resource", {"filepath": sa_file}
                )
    ...
    def test_rbac_operations(self, mcp_client):
        ...
        with namespace_context() as namespace:
            ...
            with self.temp_resource_file(role_spec) as role_file:
                role_response = mcp_client.call_tool(
                    "create_resource", {"filepath": role_file}
                )
            ...
            with self.temp_resource_file(binding_spec) as binding_file:
                ...
                assert is_valid, f"Invalid MCP response: {error}"
                assert (
                    binding_response["result"]["success"] is True
            ...
            is_valid, state = validate_kubernetes_state(
                "rolebinding", binding_name, namespace
            )
    ...
    def test_can_i_rbac_check(self, mcp_client):
        ...
        response = mcp_client.call_tool("can_i", {"verb": "get", "resource": "pods"})
        ...
        with namespace_context() as namespace:
            ...
            pod_spec = {
                ...
                "metadata": {
                    ...
                    "labels": {"app": "security-test", "created-by": "mcp-test"},
                },
                "spec": {
                    "containers": [
                        {
            ...
            with self.temp_resource_file(pod_spec) as pod_file:
                ...
                # Run security audit
                audit_response = mcp_client.call_tool(
                    "audit_pod_security",
                    {"namespace": namespace, "name": "security-test-pod"},
                )
                ...
                # Check response contents
                assert (
                    audit_response["type"] == "tool_call"
                ), "Response type should be 'tool_call'"
                assert (
                    "result" in audit_response
                ), "Response should contain 'result' field"
