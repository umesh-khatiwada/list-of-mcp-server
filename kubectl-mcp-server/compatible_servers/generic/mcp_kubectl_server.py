...
try:
    ...
    subprocess.run(
        [sys.executable, "-m", "pip", "install", "mcp>=1.4.0"],
...
class KubectlServer:
    ...
    def __init__(self):
        ...
        self.server = Server(name="kubectl-mcp-tool", version="0.1.0")
        self.setup_tools()
    ...
    def setup_tools(self):
        ...
        async def describe_resource(
            ...
            debug_file.write(
                f"describe_resource called with type={resource_type}, name={name}, namespace={namespace}\n"
            )
            debug_file.flush()
            return await self._process_query(query)
    ...
    async def run(self):
        ...
        try:
            ...
            async with stdio.stdio_server() as (read_stream, write_stream):
                ...
                debug_file.write(
                    f"Running server with initialization options: {init_options}\n"
                )
                debug_file.flush()

                # Run the server with explicit initialization
                await self.server.run(
                    read_stream, write_stream, initialization_options=init_options
                )
...
if __name__ == "__main__":
    try:
        ...
        with open(os.path.join(LOG_DIR, "startup.log"), "w") as startup_log:
            startup_log.write(
                f"Starting MCP kubectl server at {os.path.abspath(__file__)}\n"
            )
