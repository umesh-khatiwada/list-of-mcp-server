...
logger.info(
    f"Starting kubectl-mcp-tool with DEBUG={DEBUG_MODE}, LOG_LEVEL={LOG_LEVEL_NAME}"
)
...
try:
    ...
    try:
        ...
        logger.info(
            "Successfully imported kubectl_mcp_tool modules after path adjustment"
        )
    except ImportError as e:
        logger.critical(
            f"Failed to import kubectl_mcp_tool modules even with path adjustment: {e}"
        )
        sys.exit(1)
...
class StdioTransport:
    ...
    def __init__(self):
        ...
        self.debug_file.write(
            f"StdioTransport initialized at {os.path.abspath(__file__)}\n"
        )
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Put message in queue: {message.get('id', 'unknown-id')}"
                    )
                except json.JSONDecodeError as e:
                    logger.error(f"Failed to parse JSON: {line.strip()} - {e}")
                    self.debug_file.write(
                        f"JSON ERROR: {e} for input: {line.strip()}\n"
                    )
                    self.debug_file.flush()
...
class MCPServer:
    ...
    async def handle_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Handling request: {request.get('method', 'unknown')} (ID: {request.get('id', 'unknown')})"
        )
        ...
        return {
            ...
            "result": {
                "capabilities": {
                    "tools": {
                        "supported": True,
                        "properties": {"arguments": True, "descriptions": True},
                    }
                },
    ...
    async def handle_tool_call(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Handling tool/call request: {request.get('params', {}).get('name', 'unknown')}"
        )
        ...
        if tool_name == "process_natural_language":
            ...
            if "namespace" not in tool_input:
                logger.error(
                    f"Missing required parameter 'namespace' for tool: {tool_name}"
                )
                return {
                    "jsonrpc": "2.0",
                    "id": request.get("id"),
            ...
            result = await self.tools.process_query(
                f"switch to namespace {tool_input['namespace']}"
            )
            ...
            namespace = tool_input.get("namespace", "")
            query = (
                f"get deployments {f'in namespace {namespace}' if namespace else ''}"
            )
            result = await self.tools.process_query(query)
