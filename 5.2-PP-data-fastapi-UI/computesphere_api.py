...
app = FastAPI(
    title="Computesphere Agent API with UI", version="1.0.0", lifespan=lifespan
)
...
class MockComputesphereAgent:
    def __init__(self):
        self.session_id = os.getenv(
            "SESSION_ID", "5c669c6f-911a-41ac-a5bd-e11c848f2215"
        )
        self.conversations = {}
        self.memories = {}
    ...
    async def store_memory(self, content: str):
        ...
        self.memories[self.session_id].append(
            {"content": content, "timestamp": datetime.datetime.now().isoformat()}
        )
    ...
    async def retrieve_memories(self, query: Optional[str] = None):
        ...
        if query:
            ...
            return [
                m for m in session_memories if query.lower() in m["content"].lower()
            ]
    ...
    async def make_request(
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            try:
                ...
                raise HTTPException(
                    status_code=503, detail="Cannot connect to server on port 8000"
                )
            except httpx.TimeoutException:
                raise HTTPException(
                    status_code=408,
                    detail="Request to port 8000 server timed out after 3 minutes",
                )
            except Exception as e:
